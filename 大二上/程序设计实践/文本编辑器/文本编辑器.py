...
def about():
    messagebox.showinfo(title = "当前版本为1.0,欢迎使用",message = "**作者:钟昱丰\n**状态:继续努力ing")
class editor():
    ...
    def savefile(self,event =None):
        if os.path.isfile(self.t.title()):
            ...
            sname = filedialog.asksaveasfilename(title = "保存好你的文件哟",filetypes = [("保存文件","*.txt")],defaultextension = ".txt")
            if sname:
                ofp = open(sname,"w")
                ...
                if pos_line == end_line and pos_column +length > end_column:
                    ...
                    if where:
                        ...
                        if again:
                            ...
                            aa=messagebox.showinfo(title = "你终于还是放弃了",message = "你放弃了!")
                            if aa:
                                sys.exit()
    ...
    class editor():
        ...
        def savefile(self, event=None):
            if os.path.isfile(self.t.title()):
                ...
                sname = filedialog.asksaveasfilename(title="保存好你的文件哟", filetypes=[("保存文件", "*.txt")],
